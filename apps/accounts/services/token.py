...
class TokenService:
    ...
    def create_access_token(self) -> str:
        ...
        """
        print(f"[DEBUG] Creating access token for user_id: {self.user_id}")
        # --- set data to encode ---
    ...
    def update_access_token(self, token: str):
        print(f"[DEBUG] Updating access token for user_id: {self.user_id}")
        verification = UserVerification.filter(UserVerification.user_id == self.user_id).first()
        print(f"[DEBUG] Found verification record: {verification}")
        if verification:
            print(f"[DEBUG] Updating existing verification record with id: {verification.id}")
            UserVerification.update(verification.id, active_access_token=token)
        else:
            print(f"[DEBUG] Creating new verification record for user_id: {self.user_id}")
    ...
    def create_otp_token(cls):
        print(f"[DEBUG] Creating OTP token with secret key: {cls.app_config.otp_secret_key}")
        totp = TOTP(cls.app_config.otp_secret_key, interval=cls.app_config.otp_expire_seconds)
        token = totp.now()
        print(f"[DEBUG] Generated OTP token: {token}")
        return token
