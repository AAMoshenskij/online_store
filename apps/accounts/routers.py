...
from apps.core.metrics import USER_REGISTRATIONS_TOTAL, USER_LOGINS_TOTAL, USER_PROFILE_UPDATES_TOTAL
...
async def register(payload: schemas.RegisterIn = Body(**schemas.RegisterIn.examples())):
    try:
        response = AccountService.register(**payload.model_dump(exclude={"password_confirm"}))
        USER_REGISTRATIONS_TOTAL.inc()
        return response
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e) or "Registration failed")
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    try:
        response = AccountService.login(form_data.username, form_data.password)
        USER_LOGINS_TOTAL.labels(status="success").inc()
        return response
    except HTTPException as e:
        if e.status_code == status.HTTP_401_UNAUTHORIZED or e.status_code == status.HTTP_400_BAD_REQUEST:
...
async def update_me(payload: schemas.UpdateUserSchema, current_user: User = Depends(AccountService.current_user)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e) or "Profile update failed")
